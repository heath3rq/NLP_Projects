"""Explore document vectors with another TF-IDF variant.
...
def main():
    ...
    # define tf-idef (our own version)

    def tf_own(term, document):
        return 0.5 + 0.5 * document.count(term) / max(
            [document.count(t) for t in terms]
        )

    def idf_own(term, documents):
        df = 0
        for document in documents:
            if term in document:
                df += 1
        return (log(len(documents) / (df + 1))) + 1

    def tfidf_own(term, documents):
        return lambda document: tf_own(term, document) * idf_own(term, documents)

    X = np.array(
        [
            [
                ################## CHANGE THIS ##################
                tfidf_own(term, documents)(document)
                #################################################
                for term in terms
            ]
            for document in documents
    ...
    for idx in range(X.shape[0]):
        X_train = np.concatenate((X.copy()[:idx, :], X.copy()[idx + 1 :, :],), axis=0,)
        Y_train = np.concatenate((Y.copy()[:idx], Y.copy()[idx + 1 :],), axis=0,)
